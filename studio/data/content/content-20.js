...
window.STUDIO_CONTENT["20-ide-integration"] = {
  overview: `## IDE 통합 — VS Code에서 클로드 쓰기

**게임 컨트롤러**를 생각해보세요! 같은 게임이라도 키보드, 게임패드, 조이스틱 중 편한 걸로 플레이하잖아요?
클로드 코드도 터미널, VS Code, Cursor 중 **내가 편한 환경**에서 그대로 사용할 수 있어요.

### 이런 상황에서 유용해요
- **코드와 나란히 대화**: "코드를 보면서 클로드에게 질문하고 싶어" — VS Code 사이드바 패널
- **시각적 diff 확인**: "변경사항을 눈으로 확인한 후 적용하고 싶어" — 인라인 diff 미리보기
- **정확한 컨텍스트**: "이 파일의 이 부분을 가리키며 질문하고 싶어" — @-mention 기능

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | CLI vs IDE 비교, VS Code 확장, Cursor & JetBrains | 💡 개념 |
| 2 | VS Code 설치부터 @-mention, 플랜 리뷰 워크플로우까지 | 🔧 실습 |
| 3 | VS Code 버그 수정 & Cursor 코드 생성 실전 시나리오 | 💻 예제 |
...
> **핵심 요약**: CLI는 가볍고 자동화에 강하며, IDE 통합은 코드와 나란히 시각적으로 대화할 수 있습니다.
> 서버 작업은 CLI, 코드 편집 중 대화는 IDE, 상황에 맞게 전환하세요.`
    },
    {
      id: "vscode-extension",
...
**5. 스파크 아이콘 (Activity Bar)**
\`\`\`
VS Code 왼쪽 Activity Bar에 스파크 아이콘이 추가됨
클릭하면 모든 Claude Code 세션 목록을 한눈에!
-> 여러 세션을 쉽게 전환 가능
\`\`\`

**6. 마크다운 플랜 뷰**
\`\`\`
Claude가 작성한 플랜을 마크다운 문서로 볼 수 있음
-> 코멘트 지원으로 플랜에 피드백 가능
\`\`\`

**7. 네이티브 MCP 관리 (/mcp)**
\`\`\`
VS Code 안에서 MCP 서버를 관리하는 다이얼로그
-> /mcp 명령어로 서버 추가/제거/상태 확인
-> 외부 도구 없이 IDE 안에서 완결
\`\`\`
...
> **핵심 요약**: VS Code 확장은 네이티브 사이드바, 멀티탭 대화, @-mention 파일 참조, 인라인 diff 미리보기를 제공합니다.
> v2.1.70에서 스파크 아이콘, 마크다운 플랜 뷰, 네이티브 MCP 관리 등이 추가되었습니다.

#### v2.1.83~2.1.84 VSCode 개선사항

| 기능 | 설명 | 버전 |
|------|------|------|
| **"Not responding" 표시** | 백엔드가 60초 이상 무응답 시 스피너가 빨간색으로 변경 | v2.1.83 |
| **레이트 리밋 배너** | 사용량 퍼센트와 리셋 시간이 포함된 경고 배너 표시 | v2.1.84 |
| **세션 히스토리 복원** | URL/재시작 후 세션 히스토리가 정상 로드 (v2.1.83 수정) | v2.1.83 |

#### v2.1.79 새 기능

| 기능 | 설명 |
|------|------|
| **\\\`/remote-control\\\`** | VS Code 세션을 claude.ai/code로 브릿지 — 브라우저나 폰에서 이어서 작업 |
| **AI 세션 탭 제목** | 첫 메시지 기반으로 세션 탭 제목을 AI가 자동 생성 |

\\\`\\\`\\\`
비유: 게임 세이브를 클라우드에 올려서 폰으로 이어하기!

1. VS Code에서 /remote-control 실행
2. 브라우저에서 claude.ai/code 접속
3. 같은 세션을 이어서 작업!
\\\`\\\`\\\``
    },
    {
...
> **핵심 요약**: Cursor는 Agent 모드로 파일을 직접 생성/수정하고, JetBrains는 내장 터미널에서 CLI를 실행합니다.
> VS Code가 가장 풍부한 공식 통합을 제공하며, Cursor는 한 단계 더 자동화된 경험을 줍니다.`
    }
  ],
