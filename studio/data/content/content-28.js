...
> ⚠️ **v2.1.154 — /simplify와 /code-review --fix 책임 분리 안내**
>
> v2.1.154부터 \`/simplify\`(=cleanup-only)와 \`/code-review --fix\`(=풀 bug-hunting)가 명확히 분리됐어요. \`/ultrareview\`(= \`/code-review ultra\`)는 그대로 **클라우드 멀티 에이전트 병렬 리뷰**입니다. 세 가지를 함께 정리하면:
>
> | 명령 | 대상 | 적용 | 비고 |
> |------|------|------|------|
> | \`/simplify\` | cleanup만 (reuse/simplification/efficiency/altitude) | 워킹 트리에 자동 적용 | 버그 헌팅 X (v2.1.154 변경) |
> | \`/code-review\` | 풀 리뷰 (cleanup + 버그 헌팅) | 보고만 | \`--fix\`로 적용 |
> | \`/code-review --fix\` | 풀 리뷰 | 워킹 트리에 자동 적용 | 이전 \`/simplify\` alias의 동작 (v2.1.152) |
> | \`/code-review ultra\` (== \`/ultrareview\`) | 클라우드 멀티 에이전트 병렬 풀 리뷰 | 보고만 | \`--comment\`로 PR 코멘트 가능 |
>
> 즉, **\`/ultrareview\`의 위치는 그대로**입니다. 다만 형제 명령인 \`/simplify\`의 책임 범위가 좁아져서 "정리만"과 "버그까지"를 자유롭게 골라 쓸 수 있게 됐어요.`,
...
#### v2.1.119 — 다양한 호스트의 PR 리뷰

v2.1.119부터 GitHub뿐 아니라 **GitLab Merge Request, Bitbucket Pull Request, GitHub Enterprise** PR URL도 인식합니다.

\`\`\`bash
# GitLab MR
/ultrareview https://gitlab.com/group/project/-/merge_requests/42

# Bitbucket PR
/ultrareview https://bitbucket.org/team/repo/pull-requests/17

# GitHub Enterprise
/ultrareview https://github.company.com/org/repo/pull/89
\`\`\`

\`\`\`
비유: 택배사 한 곳만 받던 가게가 → 모든 택배사 받기로 확장

기존: GitHub 공식만 리뷰 가능 (다른 호스트는 manual diff 복붙 필요)
이후: GitLab/Bitbucket/GHE URL을 그대로 던져도 자동 인식
\`\`\`

> 팀이 GHE를 쓰거나 GitLab으로 마이그레이션 했어도 같은 명령으로 리뷰 흐름을 유지할 수 있어요.
> 인증은 각 호스트에 맞는 CLI(\`gh\`/\`glab\`/\`bb\`)나 토큰을 통해 자동으로 해결됩니다.
...
### v2.1.172 업데이트 — ultra 접근

| 개선 | 설명 | 버전 |
|------|------|------|
| **ultra 미로그인 시에도 표시** | \`/code-review\`에서 ultra 옵션이 claude.ai 미로그인 시에도 표시되고, 클라우드 멀티 에이전트 리뷰는 claude.ai 계정이 필요함을 안내 | v2.1.172 |`,
      checklist: [
