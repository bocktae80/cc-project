...
집을 지을 때 **건축가(디자이너)**는 도면을 그리고, **시공사(개발자)**는 도면을 보고 짓습니다. 도면을 PDF로 주고받으면 "이거 몇 px이야?" 같은 질문이 끊이지 않죠. **Figma MCP**는 건축가와 시공사가 **살아있는 공유 도면**을 보는 것과 같습니다!

### 이런 상황에서 유용해요
- **디자인 구현**: "Figma 버튼의 색상/크기를 정확히 코드로 옮기고 싶어" — 디자인 토큰 자동 추출
- **구현 검증**: "내가 만든 코드가 디자인과 정확히 같은지 확인하고 싶어" — Code to Canvas로 시각 비교
- **디자인 변경 대응**: "디자이너가 수정했는데 코드도 맞춰야 해" — 라운드트립으로 자동 감지/반영

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | Figma MCP 개념, 리모트/로컬 연결, 라운드트립 워크플로우 | 💡 개념 |
| 2 | MCP 연결 → 디자인 읽기 → Code to Canvas 실습 | 🔧 실습 |
| 3 | 라운드트립 전체 순환 + 디자인 토큰 추출 실습 | 💻 예제 |
...
| Code Connect | 코드 컴포넌트와 Figma 컴포넌트 매핑 | 도면 기호와 실제 자재 연결 |
| 다이어그램 생성 | FigJam에 다이어그램 자동 생성 | AI가 도면을 그려주기 |`,
...
4. **매핑**: Code Connect로 코드 컴포넌트 ↔ Figma 컴포넌트 연결 (v2.1.70)
5. **생성**: FigJam에 다이어그램 자동 생성 (generate_diagram)

> 건축가가 도면을 수정하면 시공사가 바로 알 수 있고, 시공 결과를 도면 위에 겹쳐서 차이를 확인할 수 있는 것과 같습니다!

> **핵심 요약**: Figma MCP는 디자인(도면)과 코드(시공)를 실시간으로 연결합니다. 읽기(토큰 추출), 쓰기(Code to Canvas), 비교(차이 감지), 매핑(Code Connect), 생성(다이어그램) 5가지 기능을 제공합니다.`
    },
    {
      id: "remote-vs-local",
      title: "리모트 vs 로컬 MCP",
...
> **핵심 요약**: 리모트 MCP는 OAuth 간편 인증 + Code to Canvas 지원으로 대부분의 사용자에게 추천됩니다. 로컬 MCP는 API 키 직접 발급이 필요하고 읽기 전용이지만, 오프라인/커스텀이 가능합니다.`
    },
    {
      id: "round-trip",
      title: "라운드트립 워크플로우",
...
> **핵심 요약**: 라운드트립은 디자인 읽기 → 코드 작성 → Code to Canvas로 비교 → 차이 수정 → 재확인의 순환입니다. 수동 비교 대신 자동으로 디자인과 코드를 정확히 일치시킵니다.`
    }
  ],
...
> **팁**: 처음에는 전체 페이지가 아니라 **작은 컴포넌트 하나**부터 시작하세요. 버튼, 카드, 인풋 박스 등 작은 단위로 연습한 후 큰 레이아웃에 도전하는 것이 좋습니다!`,
      checklist: [
        ...
        "Code Connect와 다이어그램 생성 등 v2.1.70 신규 기능을 알고 있다"
      ]
    },
    {
      id: "design-tokens",
