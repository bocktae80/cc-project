...
window.STUDIO_CONTENT["15-browser-automation"] = {
  overview: `## 브라우저 자동화 — AI가 브라우저를 조종하기

리모컨으로 TV를 조종하듯, 클로드가 **브라우저를 직접 조종**할 수 있어요!
채널 돌리기(페이지 이동), 볼륨 조절(폼 입력), 화면 캡처(스크린샷)까지 전부 가능합니다.

### 이런 상황에서 유용해요
- **UI 테스트**: "로그인 폼에 자동으로 입력하고 결과를 확인해줘" — 반복 테스트를 자동화
- **디자인 검증**: "Figma 시안과 실제 화면을 비교해줘" — 스크린샷으로 차이점 분석
- **디버깅**: "콘솔 에러와 네트워크 요청 실패를 확인해줘" — 브라우저 뒤에서 일어나는 일 점검

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | Chrome 확장 설치와 CLI 연결 원리 | 💡 개념 |
| 2 | 페이지 이동, 클릭, 폼 입력, 스크린샷 실습 | 🔧 실습 |
| 3 | 디자인 검증 & 디버깅 실전 시나리오 | 💻 예제 |
...
> **핵심 요약**: navigate(이동), click(클릭), type(입력), screenshot(캡처), evaluate(JS 실행), GIF 녹화 6가지 기능을 제공합니다.
> Chrome 브라우저 안에서만 동작하며, 데스크톱 앱이나 다른 브라우저는 조작할 수 없습니다.`
    },
    {
...
> **핵심 요약**: 브라우저 자동화로 Figma 디자인과 실제 빌드 결과를 스크린샷으로 비교하고,
> 콘솔 에러와 네트워크 요청 실패까지 한 번에 점검하는 디자인 검증 워크플로우를 구성할 수 있습니다.`
    }
  ],
