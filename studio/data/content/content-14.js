...
### 이런 상황에서 유용해요
- **출시 전 점검**: "배포 전에 보안 허점 없는지 한번 훑어보고 싶어" — AI가 코드 맥락을 읽고 숨은 취약점까지 찾아줘요
- **코드 리뷰 강화**: "PR 올렸는데 보안 관점에서 놓친 건 없을까?" — SQL 인젝션, XSS 등 복합 취약점을 추론
- **보안 학습**: "OWASP Top 10이 뭔지, 내 코드에 해당되는 건 없는지 알고 싶어" — 취약 코드와 안전한 코드를 비교하며 학습

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | OWASP Top 10과 AI 스캔 vs 기존 정적분석 차이 | 💡 개념 |
| 2 | 취약점 이해, 보안 스캔 실행, 수정 사이클 실습 | 🔧 실습 |
| 3 | 취약 코드 분석 + 스캔/수정 워크플로우 + 체크리스트 | 💻 예제 |
...
> **핵심 요약**: 코드 보안 취약점은 대부분 "깜빡함, 모름, 귀찮음, 복잡함"에서 생깁니다. AI 경비원(Claude)은 CCTV(기존 도구)와 달리 코드의 맥락을 이해하고 수상한 행동을 추론해서, OWASP Top 10 같은 주요 취약점을 발견합니다.`
    },
    {
...
> **핵심 요약**: 기존 정적분석은 규칙 매칭(CCTV)이고, AI 스캔은 맥락 이해(경비원)입니다. AI는 새로운 취약점 추론, 복합 취약점 연결, 상세 수정 제안이 가능하지만 속도는 느립니다. 최고의 보안은 두 접근법을 함께 쓰는 것입니다.

### v2.1.160 업데이트 — 셸 시작 파일 보호

| 개선 | 설명 | 버전 |
|------|------|------|
| **셸 시작 파일 쓰기 전 프롬프트** | \`.zshenv\`/\`.zlogin\`/\`.bash_login\` 및 \`~/.config/git/\`에 쓰기 전 **확인 프롬프트** — 의도치 않은 명령 실행으로 이어질 수 있는 경로를 보호 | v2.1.160 |

### v2.1.169 업데이트 — 신뢰 안 된 설정의 인증서 경로 차단

| 기능 | 설명 | 버전 |
|------|------|------|
| **OTEL 클라이언트 인증서 신뢰 확인** | 신뢰하지 않은(untrusted) 프로젝트 설정이 OTEL 클라이언트 인증서 경로를 **신뢰 확인 없이 지정하지 못하도록** 차단 — 악의적 프로젝트가 텔레메트리(원격 측정) 경로를 몰래 가로채는 것을 방지하는 보안 강화 | v2.1.169 |`
    }
  ],
