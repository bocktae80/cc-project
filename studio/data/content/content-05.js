...
| 변경 | 설명 | 버전 |
|------|------|------|
| **\`● N running\` 인디케이터** | \`/agents\` 화면에서 에이전트 타입 옆에 **현재 실행 중인 서브에이전트 개수**를 라이브로 표시 — 어떤 에이전트가 몇 개 동시에 돌아가는지 한눈에 파악 | v2.1.97 |
| **서브에이전트 cwd 누수 수정** | worktree 격리 또는 \`cwd:\` 오버라이드 서브에이전트가 자신의 작업 디렉토리를 부모 세션의 Bash 도구로 누출시키던 문제 해결 | v2.1.97 |
| **429 에이전트 정지 수정** | Retry-After가 긴 429 응답을 받았을 때 에이전트가 조용히 멈춘 것처럼 보이던 문제 — 이제 에러가 즉시 표면화 | v2.1.94 |

\`\`\`
/agents 화면 (v2.1.97+):

  code-reviewer       ● 2 running
  test-writer         ● 1 running
  docs-updater
  security-auditor    ● 3 running

→ 라이브 카운트로 백그라운드 에이전트 활동이 한눈에!
\`\`\`
...
| 변경 | 설명 | 버전 |
|------|------|------|
| **네임드 에이전트 @ 멘션** | 이름 있는 서브에이전트가 \`@\` 멘션 타입어헤드 추천에 표시 | v2.1.89 |
...
### v2.1.172 업데이트 — 중첩 서브에이전트

| 개선 | 설명 | 버전 |
|------|------|------|
| **중첩 서브에이전트(5단계)** ⭐⭐⭐ | 서브에이전트가 **자체 서브에이전트를 재귀적으로 스폰** 가능 — 최대 5단계 깊이. 멀티에이전트 토폴로지를 트리 구조로 확장 | v2.1.172 |`
    }
  ],
