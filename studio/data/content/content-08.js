...
> **비유**: "게임 매크로에서 외부 프로그램 실행을 차단" — 매크로(스킬)가 \\\`command\\\` 블록으로 셸 명령을 실행하는 것을 막습니다. 보안이 중요한 환경에서 유용!

#### v2.1.86 스킬 목록 개선

- **설명 250자 캡**: \`/skills\` 목록에서 스킬 설명이 **250자로 잘림** → 컨텍스트 사용량 감소
- **알파벳 정렬**: \`/skills\` 메뉴가 **알파벳순 정렬**로 변경 → 스킬이 많을 때 더 쉽게 찾기

\`\`\`
비유: 게임 매크로 목록 정리

기존: 매크로가 등록 순서대로 나열 → 찾기 어려움, 설명이 길어서 화면 가득
이후: 가나다순 정렬 + 설명 한 줄 요약 → 빠르게 원하는 매크로 찾기!
\`\`\`

> 스킬 설명이 250자를 넘으면 잘리므로, 핵심을 앞부분에 작성하세요!

#### v2.1.108 Skill 도구로 내장 슬래시 커맨드 호출

**모델이 Skill 도구를 통해 \`/init\`, \`/review\`, \`/security-review\` 같은 내장 슬래시 커맨드를 직접 발견·호출**할 수 있게 됐습니다. 이전에는 사용자가 타이핑해야만 실행되던 커맨드가, 이제 AI가 필요한 맥락에서 **스스로 불러서 쓸 수 있어요**.

\`\`\`
비유: 매크로 안에서 다른 매크로를 부르기

기존: 사용자가 /review 를 직접 입력해야만 리뷰 실행
이후: "이 변경 리뷰해줘" → AI가 내부적으로 /review 를 호출
     → 사용자는 자연어로만 요청하면 됨
\`\`\`

**어떤 커맨드가 호출 가능?**

| 커맨드 | 용도 | AI 자동 호출 시나리오 |
|--------|------|---------------------|
| \`/init\` | CLAUDE.md 초기화 | "이 프로젝트 CLAUDE.md 만들어줘" |
| \`/review\` | PR/변경사항 리뷰 | "내 변경사항 리뷰해줘" |
| \`/security-review\` | 보안 리뷰 | "보안 이슈 있는지 봐줘" |

\`\`\`
# 예시 대화
사용자: "내가 방금 짠 코드 보안 문제 있는지 봐줘"
  ↓
AI: (내부적으로) Skill 도구 호출 → /security-review 실행
  ↓
리뷰 결과가 즉시 표시
\`\`\`

> **주의**: 이 기능은 AI 자율 실행이므로, 사용자가 원치 않는 자동 호출을 막으려면 해당 스킬 프론트매터에 \`disable-model-invocation: true\`를 추가하세요 (커스텀 스킬에만 해당).

#### v2.1.108 \`/recap\` 커맨드 & \`/undo\` alias

- **\`/recap\`**: 세션에 복귀했을 때 **이전 작업 컨텍스트를 요약**해서 보여줍니다. \`/config\`에서 활성화, 수동 호출 가능, \`CLAUDE_CODE_ENABLE_AWAY_SUMMARY\`로 강제
- **\`/undo\`** = \`/rewind\` alias — 직관적인 이름으로도 호출 가능

#### v2.1.121 \`/skills\` type-to-filter 검색 박스

\`/skills\` 메뉴 상단에 **즉석 필터 검색 박스**가 추가됐습니다. 긴 스킬 목록에서 스크롤하지 않고 키워드만 입력해 빠르게 찾을 수 있습니다.

\`\`\`
/skills
┌──────────────────────────────────────────┐
│ Search: ses_                             │ ← 타이핑 즉시 필터
├──────────────────────────────────────────┤
│ wb-session         워크북 세션 시작/종료 │
│ session-replay     세션 리플레이 도구    │
│ debug-session      디버그 세션 진입      │
└──────────────────────────────────────────┘
\`\`\`

\`\`\`
비유: 도서관 검색대 (지팡이로 책장 짚기 vs 단말기에 제목 입력)

기존 (v2.1.120 이하): 알파벳순 목록을 위↓아래↑로 스크롤
이후 (v2.1.121+):    "ses" 입력 → 매칭 스킬만 표시 → Enter
\`\`\`
...
> **핵심 요약**: 커스텀 스킬은 \`.claude/skills/이름/SKILL.md\` 디렉토리 구조로 만듭니다. 프론트매터(YAML)로 호출 제어·도구 제한을 설정하고, 본문에 프롬프트를 작성합니다. \$ARGUMENTS로 인자를 전달받을 수 있습니다. v2.1.86부터 \`/skills\` 목록이 알파벳순 정렬되고 설명은 250자로 캡됩니다.`
    },
    {
      id: "skill-lifecycle",
...
### v2.1.163 업데이트 — \`\\$\` 이스케이프 & /plugin list

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`\\$\` 이스케이프 문법** | 슬래시 커맨드 본문에서 **숫자 앞 리터럴 \`$\`**를 넣을 때 \`\\$\`로 이스케이프 — 예: \`\\$5\`처럼 써야 인자 \`$5\`로 해석되지 않고 문자 그대로 출력됩니다 | v2.1.163 |
| **\`/plugin list\`** | 설치된 플러그인 목록 조회(\`--enabled\`/\`--disabled\` 필터) — 튜토리얼 17 참고 | v2.1.163 |

### v2.1.169 업데이트 — 번들 스킬 숨기기

| 기능 | 설명 | 버전 |
|------|------|------|
| **\`disableBundledSkills\` 설정** | 번들로 딸려오는 스킬·워크플로우·내장 슬래시 명령을 **모델에게서 숨김** — 내 프로젝트 전용 스킬만 노출하고 싶거나 명령 목록을 깔끔히 유지하고 싶을 때 사용. \`CLAUDE_CODE_DISABLE_BUNDLED_SKILLS\` 환경변수로도 설정 | v2.1.169 |

### v2.1.174~178 업데이트 — 중첩 스킬 & 핫리로드

| 개선 | 설명 | 버전 |
|------|------|------|
| **중첩 \`.claude/skills\` 충돌 해소** | 해당 서브트리 파일 작업 시 중첩 \`.claude/skills\` 자동 로드. 이름 충돌 시 한쪽을 가리지 않고 \`<dir>:<name>\` 한정자로 **둘 다 사용 가능** 유지 | v2.1.178 |
| **스킬 핫리로드 변경분만 재공지** | \`/reload-skills\`·SessionStart \`reloadSkills\`가 전체 스킬 목록 재전송 대신 **변경된 스킬만** 모델에 재공지(컨텍스트 절약) | v2.1.174 |`
    }
  ],
