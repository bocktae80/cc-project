...
### v2.1.178 업데이트 — 중첩 .claude 해석 & 트리거

| 개선 | 설명 | 버전 |
|------|------|------|
| **중첩 \`.claude/\` 근접 우선 해석** | agent/workflow/output-style 이름 충돌 시 작업 디렉토리에서 **가장 가까운 \`.claude/\` 우선**. project-scope 워크플로우는 리포 루트가 아닌 가장 가까운 기존 \`.claude/workflows/\`에 저장 | v2.1.178 |
| **workflow 키워드 명시 구문 트리거** | workflow 프롬프트 키워드가 단어 언급이 아닌 **명시 구문**(\`run a workflow\`, \`workflow:\`)에서만 트리거 + 입력창 purple shimmer 하이라이트 (v2.1.157~160에서 \`workflow\` 단어 트리거를 끈 뒤 명시 구문 한정으로 부분 복원) | v2.1.178 |
| **\`/workflows\` 턴 진행 중 즉시 열림** | 턴이 진행 중이어도 \`/workflows\`가 즉시 열림(이전엔 턴 종료까지 대기) | v2.1.169 |`
    }
  ]
};
