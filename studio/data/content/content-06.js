...
### v2.1.140~2.1.153 훅 개선사항

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`MessageDisplay\` 신규 훅 이벤트** | 어시스턴트 메시지가 화면에 표시되기 직전 훅이 **텍스트를 변환하거나 숨길** 수 있음 — 줄단위 마스킹, 비속어 필터, 다국어 번역, 사내 용어 치환 등에 활용 | v2.1.152 |
| **\`SessionStart\` 훅 \`reloadSkills: true\` 반환** | SessionStart 훅이 \`hookSpecificOutput.reloadSkills: true\`를 반환하면 **스킬 디렉토리 재스캔** — 훅이 스킬 설치/갱신 후 같은 세션에서 즉시 사용 가능 | v2.1.152 |
| **\`SessionStart\` 훅 \`sessionTitle\` 반환** | SessionStart/Resume 시 훅이 \`hookSpecificOutput.sessionTitle\`로 **세션 제목을 직접 지정** (UserPromptSubmit에서만 되던 것 → 시작 시점부터 가능) | v2.1.152 |
| **\`Stop\`/\`SubagentStop\` 훅 입력에 \`background_tasks\`/\`session_crons\`** | 종료 시점에 살아있는 백그라운드 셸과 스케줄된 작업 목록을 훅이 직접 확인 — \`/exit\` 막거나 사용자에게 경고 발송 가능 | v2.1.145 |
| **\`terminalSequence\` 훅 JSON 출력 필드** | 훅 응답에 \`terminalSequence\` 추가 → 데스크탑 알림, 윈도우 타이틀, 벨 사운드를 컨트롤하는 터미널이 없어도 발동 가능 | v2.1.141 |
| **\`CLAUDE_CODE_STOP_HOOK_BLOCK_CAP\`** | Stop 훅이 반복적으로 \`block\`을 반환해 무한 루프에 빠지는 경우, 기본 8회 후 자동으로 턴 종료 — 환경변수로 임계값 조정 | v2.1.143 |
| **Stop 훅 반복 \`block\` 자동 종료** | Stop 훅이 8회 연속 \`block\`을 반환하면 경고 후 턴 종료 (기본 8, \`CLAUDE_CODE_STOP_HOOK_BLOCK_CAP\`로 변경) | v2.1.143 |
| **prompt/agent 타입 훅 \`SessionStart\`/\`Setup\`/\`SubagentStart\` 에러 메시지 개선** | 잘못된 타입을 지정하면 "command 타입을 사용하세요" 메시지를 명확히 표시 | v2.1.142 |
| **Skill/슬래시 \`disallowed-tools\` 프론트매터** | 스킬 또는 슬래시 명령 프론트매터에 \`disallowed-tools\`를 선언하면 활성화 중 해당 도구를 모델에서 **제거** (튜토리얼 8에서 상세) | v2.1.152 |

#### \`MessageDisplay\` — 표시 직전 가공/숨김
    ...
    "MessageDisplay": [
      {
        "hooks": [{
          "type": "command",
          "command": ".claude/hooks/redact-message.sh"
        }]
      }
    ]
  }
...
\`\`\`bash
#!/bin/bash
# .claude/hooks/redact-message.sh — 어시스턴트 메시지에서 비밀 키 패턴 마스킹
INPUT=$(cat)
TEXT=$(echo "$INPUT" | jq -r '.message_text // ""')

# AWS / GitHub / Slack 토큰 마스킹
REDACTED=$(echo "$TEXT" \\
  | sed -E 's/AKIA[0-9A-Z]{16}/AKIA****/g' \\
  | sed -E 's/ghp_[a-zA-Z0-9]{36}/ghp_****/g' \\
  | sed -E 's/xoxb-[0-9-]+-[0-9-]+-[a-zA-Z0-9]+/xoxb-****/g')
...
\`\`\`
비유: 학급 게시판 게시 직전에 검열관이 검토

기존(v2.1.151 이전): 어시스턴트 메시지는 화면에 그대로 표시됨
이후(v2.1.152+): MessageDisplay 훅 → 표시 직전 가공/숨김 가능
              → 비밀 키 마스킹, 금기어 필터, 다국어 자동 번역, 어조 통일
\`\`\`

> \`PostToolUse\`의 \`updatedToolOutput\`이 **도구 결과**를 가공한다면, \`MessageDisplay\`는 **어시스턴트 메시지 본문**을 가공합니다. 둘은 다른 계층이에요.

#### \`SessionStart\`의 \`reloadSkills\` — 훅이 스킬 설치 후 즉시 사용

\`\`\`bash
#!/bin/bash
# .claude/hooks/install-team-skills.sh — SessionStart 훅
# 팀 공유 스킬을 동기화한 뒤 같은 세션에서 즉시 인식

git -C ~/.claude/team-skills pull --quiet
ln -sf ~/.claude/team-skills/skills/* ~/.claude/skills/

# reloadSkills: true → 세션 재시작 없이 스킬 디렉토리 재스캔
jq -n '{
  hookSpecificOutput: {
    hookEventName: "SessionStart",
    sessionTitle: "[Team Skills Synced]",
    reloadSkills: true
  }
}'
\`\`\`

> 이전엔 SessionStart 훅이 스킬을 설치해도 사용자가 \`/reload-skills\`(v2.1.152 신규 명령)를 직접 실행하거나 재시작해야 했습니다. 이제 훅이 직접 재스캔을 트리거할 수 있어요.

#### Stop 훅에서 백그라운드 작업 확인

\`\`\`json
// Stop 훅 JSON 입력 예시 (v2.1.145+)
{
  "session_id": "abc-123",
  "background_tasks": [
    { "id": "bg-001", "type": "shell", "cmd": "npm run dev", "running": true },
    { "id": "bg-002", "type": "agent", "name": "code-reviewer", "status": "blocked" }
  ],
  "session_crons": [
    { "id": "cron-001", "schedule": "*/15 * * * *", "next_run": "..." }
  ]
}
\`\`\`

> 세션 종료 시 살아있는 백그라운드 셸/에이전트/스케줄된 작업을 훅이 직접 확인하고, 필요하면 \`{"decision": "block", "reason": "..."}\`로 종료를 막을 수 있습니다.

### v2.1.127~2.1.139 훅 개선사항

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`args: string[]\` exec form** | command 타입 훅에서 \`args\`로 인자를 배열로 전달 — **셸 없이 직접 spawn**하므로 경로 placeholder(\`\${CLAUDE_PROJECT_DIR}\` 등)에 따옴표가 필요 없음 (공백 들어간 경로도 안전) | v2.1.139 |
| **\`continueOnBlock\` for PostToolUse** | PostToolUse 훅 설정에 \`"continueOnBlock": true\` — 훅이 도구 결과를 거부하면 거부 이유를 Claude에게 **피드백하고 턴 계속** (이전엔 즉시 차단) | v2.1.139 |
| **\`effort.level\` 입력 + \`$CLAUDE_EFFORT\` 환경변수** | 모든 훅 JSON 입력에 \`effort.level\` 필드 추가 + Bash 도구 명령에서 \`$CLAUDE_EFFORT\` 사용 가능 — 현재 effort에 따라 훅 로직 분기 가능 | v2.1.133 |
| **\`$CLAUDE_CODE_SESSION_ID\` Bash 환경변수** | Bash 도구 서브프로세스 환경에 \`CLAUDE_CODE_SESSION_ID\` 주입 — 훅에 전달되는 \`session_id\`와 매칭 (세션별 로그 라우팅) | v2.1.132 |
| **터미널 출력 충돌 수정** | 훅이 터미널에 직접 출력하면 화면 위 대화형 프롬프트가 깨지던 버그 — 훅은 이제 터미널 접근 없이 실행됨 | v2.1.139 |
| **\`Skill(name *)\` 와일드카드** | 권한 규칙 \`Skill(name *)\`이 \`Bash(ls *)\`처럼 **prefix 매칭** 정상 동작 (이전엔 정확 일치만) | v2.1.139 |
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [{
          ...
          "args": ["--project", "\${CLAUDE_PROJECT_DIR}", "--file", "\${tool_input.file_path}"]
        }]
      }
    ]
  }
...
이전: command: "fmt.sh --project '\${CLAUDE_PROJECT_DIR}'"
      → 셸이 파싱 → 경로에 공백/'/'/'$' 있으면 깨짐
이후: command + args: [...] → exec 직행
      → 경로가 어떻든 인자 그대로 전달 (인용 불필요)
\`\`\`

> Windows 경로(\`C:\\Users\\My Documents\`)나 한글 경로처럼 공백·특수문자가 포함된 환경에서 훅 깨짐을 막는 결정적 옵션입니다.

#### \`continueOnBlock\`으로 거부 이유를 학습 신호로
...
\`\`\`bash
#!/bin/bash
# lint-gate.sh — 작성된 파일이 lint 실패 시 거부
FILE=$(jq -r '.tool_input.file_path')
if ! npx eslint "$FILE" > /tmp/lint.log 2>&1; then
  jq -n --rawfile log /tmp/lint.log '{
    decision: "block",
    reason: ("ESLint 실패:\\n" + $log + "\\n해당 파일을 다시 수정해 주세요.")
  }'
  exit 0
...
| 모드 | 차단 후 동작 |
|------|------------|
| \`continueOnBlock: false\` (기본) | 도구 결과 폐기, 턴 종료 — 사용자가 다시 지시해야 함 |
| \`continueOnBlock: true\` | 거부 이유가 모델 컨텍스트에 주입 → **Claude가 자동으로 재시도**(예: lint 에러 보고 수정) |

> 자동 수정 가능한 결함(lint, 포맷, import 정렬)을 잡는 PostToolUse 훅에 켜두면 "거부 → 학습 → 재시도"가 자율 루프로 돌아갑니다.

#### \`effort.level\` / \`$CLAUDE_EFFORT\`로 모드 분기

\`\`\`bash
#!/bin/bash
# .claude/hooks/policy.sh — effort에 따른 검증 강도 조절
EFFORT=$(jq -r '.effort.level // env.CLAUDE_EFFORT // "medium"')
...
case "$EFFORT" in
  ...
  high|xhigh|max)
    # 보안 스캔 + 타입체크 + 테스트 풀세트
    .claude/scripts/deep-verify.sh
    ;;
esac
\`\`\`

> Auto mode가 효율 절감을 위해 effort를 자동 조정할 때 훅이 같은 신호를 보고 따라가도록 만드는 패턴입니다.

#### \`Skill(name *)\` 권한 prefix 매칭
...
// .claude/settings.json
{
  "permissions": {
    "allow": [
      "Skill(name wb-*)",      // ✓ wb-session, wb-plan, wb-quality... 모두 허용
      "Skill(name camfit-*)"   // ✓ 캠핏 플러그인 스킬 일괄 허용
    ]
  }
}
\`\`\`

> 이전엔 정확 일치만 되어 스킬마다 따로 등록해야 했지만, 이제 \`Bash(ls *)\`와 동일한 prefix 매칭으로 **플러그인 단위 일괄 허용**이 가능합니다.

### v2.1.121~2.1.126 훅 개선사항

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`PostToolUse\` \`updatedToolOutput\` 일반화** | \`hookSpecificOutput.updatedToolOutput\`으로 **모든 도구**의 출력을 훅이 교체 가능 (이전엔 MCP 도구 전용) — Bash/Read/Edit/MCP 어떤 도구든 결과를 가공해서 넘길 수 있음 | v2.1.121 |
| **\`claude_code.skill_activated\` OTEL 이벤트** | OpenTelemetry 이벤트가 사용자 입력 슬래시 명령에서도 발동하며 \`invocation_trigger\` 속성 추가 — \`"user-slash"\` / \`"claude-proactive"\` / \`"nested-skill"\` 구분 가능 | v2.1.126 |

#### \`updatedToolOutput\`으로 도구 결과 후처리

\`\`\`json
// .claude/hooks/redact.json — PostToolUse 훅
{
  "hooks": {
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": ".claude/hooks/redact.sh"
        }]
      }
    ]
  }
...
\`\`\`bash
#!/bin/bash
# .claude/hooks/redact.sh — Bash 도구 출력에서 토큰 마스킹
INPUT=$(cat)
RAW=$(echo "$INPUT" | jq -r '.tool_response.stdout // ""')

# AWS 키, OAuth 토큰 패턴 마스킹
REDACTED=$(echo "$RAW" \\
  | sed -E 's/AKIA[0-9A-Z]{16}/AKIA****REDACTED****/g' \\
  | sed -E 's/sk-ant-[a-zA-Z0-9-]+/sk-ant-****REDACTED****/g')

# updatedToolOutput으로 교체
jq -n --arg out "$REDACTED" '{
  hookSpecificOutput: {
...
기존(v2.1.120 이전): MCP 도구만 검열 가능 (Slack/Notion/Linear)
이후(v2.1.121+): Bash, Read, Edit 등 **모든 도구의 결과를 가공**해서 표시
              → 비밀 키 마스킹, 로그 압축, 다국어 번역, 포맷 통일까지 가능
\`\`\`

> 이 변화는 **민감 정보 마스킹**, **출력 정규화**, **품질 게이트**(예: lint 결과를 합쳐서 단일 메시지로) 같은 후처리 파이프라인을 훅 한 줄로 구현할 수 있게 합니다.

#### \`invocation_trigger\`로 스킬 호출 경로 분석

\`\`\`json
// OTEL 이벤트 페이로드 예시 (v2.1.126)
{
  "name": "claude_code.skill_activated",
  "attributes": {
    "skill_name": "wb-session",
    "invocation_trigger": "user-slash",  // 또는 "claude-proactive" / "nested-skill"
    "session_id": "..."
  }
}
\`\`\`
...
| 트리거 | 의미 | 활용 |
|--------|------|------|
| \`user-slash\` | 사용자가 \`/skill-name\` 직접 입력 | 명시적 사용 빈도 측정 |
| \`claude-proactive\` | Claude가 description 기반으로 자율 호출 | 프롬프트 매칭 정확도 평가 |
| \`nested-skill\` | 다른 스킬이 Skill 도구로 중첩 호출 | 스킬 의존성/연계 분석 |

> 사내 OTEL 백엔드에서 이 속성으로 그루핑하면 "Claude가 잘 골라 쓰는 스킬"과 "사용자가 직접 부르는 스킬"을 구분해 description 개선 우선순위를 매길 수 있습니다.

### v2.1.116~2.1.119 훅 개선사항

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`mcp_tool\` 훅 타입** | 훅에서 \`type: "mcp_tool"\`로 MCP 서버 도구를 직접 호출 — Bash/curl 우회 없이 Slack/Notion/Linear 등을 바로 트리거 | v2.1.118 |
| **\`PostToolUse\` \`duration_ms\` 입력** | \`PostToolUse\`/\`PostToolUseFailure\` 훅 입력에 \`duration_ms\`(도구 실행 시간) 포함 — 권한 프롬프트와 PreToolUse 훅 시간은 제외한 순수 실행 시간 | v2.1.119 |
| **Agent frontmatter 훅 \`--agent\` 메인 스레드 동작** | 에이전트 프론트매터의 \`hooks:\`가 \`--agent\`로 메인 스레드 에이전트 실행 시에도 발동 (이전엔 서브에이전트에서만) | v2.1.116 |
| **Agent 훅 이벤트 확장** | 에이전트 타입 훅이 \`Stop\`/\`SubagentStop\` 외 다른 이벤트에도 설정 가능하도록 수정 ("Messages are required for agent hooks" 에러 해결) | v2.1.118 |
| **\`prompt\` 훅 재발동 방지** | 에이전트 훅 검증기 서브에이전트가 만든 도구 호출에 \`prompt\` 훅이 재발동하던 문제 해결 | v2.1.118 |
| **빈 응답 페이로드 트랜스크립트 정리** | 응답 없이 종료하는 비동기 \`PostToolUse\` 훅이 빈 엔트리를 트랜스크립트에 쓰던 문제 수정 | v2.1.119 |

#### \`duration_ms\`로 느린 도구 추적

\`\`\`bash
#!/bin/bash
# .claude/hooks/perf-log.sh — PostToolUse 훅
INPUT=$(cat)
...
if [ "$DURATION" -gt 1000 ]; then
  echo "[$(date)] $TOOL took \${DURATION}ms" >> .claude/slow-tools.log
fi
\`\`\`

\`\`\`
비유: 교실 시계가 각 활동 시간을 자동 측정

기존: PostToolUse 훅 → "도구 실행됐어"만 알려줌
이후: PostToolUse 훅 → "도구 실행됐어 + 357ms 걸렸어" → 병목 추적 가능!
\`\`\`

> 권한 프롬프트 대기시간이나 PreToolUse 훅 실행시간은 \`duration_ms\`에서 **제외**됩니다.
> 즉 순수한 도구 자체 실행 시간이라 성능 분석에 곧바로 활용할 수 있어요.

### v2.1.105~2.1.108 훅 개선사항

| 개선 | 설명 | 버전 |
|------|------|------|
| **PreCompact 훅 블로킹 지원** | \`PreCompact\` 훅이 \`exit 2\` 또는 \`{"decision":"block"}\`을 반환하면 **컨텍스트 압축을 차단**할 수 있음 — 중요한 대화/산출물을 보호하거나 압축 전 백업 강제 | v2.1.105 |
| **플러그인 \`monitors\` 백그라운드 훅** | 플러그인이 \`monitors\` 매니페스트 키로 백그라운드 모니터 스크립트를 선언 → 세션 시작 또는 스킬 호출 시 **자동으로 arm** (장기 이벤트 감시 훅의 신 패턴) | v2.1.105 |

#### PreCompact 블로킹 예제

\`\`\`python
#!/usr/bin/env python3
# .claude/hooks/guard-compact.py — PreCompact 훅
import json, sys, os

data = json.load(sys.stdin)
transcript = data.get("transcript", "")

# 진행 중인 중요 작업이 있으면 압축 차단
if "DO_NOT_COMPACT" in transcript or os.path.exists(".no-compact.lock"):
    print(json.dumps({
        "decision": "block",
        "reason": "진행 중인 작업이 있어 압축을 연기합니다."
    }))
    sys.exit(0)
\`\`\`

\`\`\`
비유: 교실 칠판을 지우기 전에 손 들기!

기존: 컨텍스트가 차면 자동으로 압축 (되돌릴 수 없음)
이후: PreCompact 훅이 "잠깐!" 하면 압축이 멈춤
     → 중요한 내용을 백업하거나 사용자에게 확인받을 수 있음
\`\`\`

> \`PreCompact\` 훅은 \`/compact\` 수동 호출과 자동 압축 양쪽에서 모두 발동합니다.

### v2.1.94~2.1.97 훅 개선사항

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`sessionTitle\` 설정** | \`UserPromptSubmit\` 훅이 \`hookSpecificOutput.sessionTitle\`을 반환하면 **세션 제목을 동적으로 지정** — 첫 프롬프트 내용으로 탭 제목 자동 설정 등에 활용 | v2.1.94 |
| **Stop/SubagentStop 장기세션 수정** | prompt 타입 Stop/SubagentStop 훅이 긴 세션에서 실패하던 문제 해결, 평가기 API 에러가 "JSON validation failed" 대신 실제 메시지를 표시 | v2.1.97 |
| **plugin 스킬 훅 인식** | YAML frontmatter에 선언된 플러그인 스킬 훅이 조용히 무시되던 문제 수정 | v2.1.94 |
| **빈 훅 엔트리 축소** | 트랜스크립트 크기를 줄이기 위해 빈 훅 엔트리를 건너뛰고, 사전 편집 파일 복사본 저장량에 상한 적용 | v2.1.97 |

#### \`sessionTitle\` 훅 예제

\`\`\`python
#!/usr/bin/env python3
# .claude/hooks/set-title.py — UserPromptSubmit 훅
import json, sys
...
> 첫 프롬프트를 입력하면 세션 제목이 자동으로 갱신되어, 터미널 탭이나
> cmux 워크스페이스에서 어떤 작업 중인지 한눈에 파악할 수 있어요.
...
| 개선 | 설명 | 버전 |
|------|------|------|
| **\`Stop\`/\`SubagentStop\` 훅 \`additionalContext\` 반환** | 훅이 \`hookSpecificOutput.additionalContext\`를 반환하면 **훅 에러로 분류되지 않고** Claude에 피드백을 주며 **턴을 계속 진행**한다 — "조금 더 점검해줘" 같은 유도에 활용 | v2.1.163 |
| **\`tool_decision\` 텔레메트리 \`tool_parameters\`** | \`OTEL_LOG_TOOL_DETAILS=1\`이면 \`tool_decision\` 이벤트에 bash 명령·MCP/스킬 이름 등 파라미터가 포함 | v2.1.157 |
| **\`OTEL_RESOURCE_ATTRIBUTES\` 메트릭 라벨** | 값이 메트릭 데이터포인트의 라벨로 포함되어 팀·레포 등 커스텀 차원으로 사용량을 분석 가능 | v2.1.161 |

### v2.1.172~176 업데이트 — Hooks & OTEL

| 개선 | 설명 | 버전 |
|------|------|------|
| **Hook \`if\` 경로 매칭 수정** | Read/Edit/Write 경로 조건이 정상 매칭 — \`Edit(src/**)\`, \`Read(~/.ssh/**)\`, \`Read(.env)\` 같은 문서화된 패턴 동작 | v2.1.176 |
| **\`lines_of_code.count\` OTEL model 속성** | \`claude_code.lines_of_code.count\` 메트릭에 \`model\` 속성 추가 — 모델별 추가/삭제 라인 분해 | v2.1.172 |`
    },
    {
      id: "settings-config",
...
#### 훅 타입: 5가지 (v2.1.118 \`mcp_tool\` 추가)

| 타입 | 설명 | 용도 | 도입 |
|------|------|------|------|
| \`command\` | 셸 명령어/스크립트 실행 | 가장 기본적 | v2.1.70 |
| \`http\` | HTTP POST 요청 전송 | 서버 기반 검증 | v2.1.70 |
| \`prompt\` | LLM에게 프롬프트 평가 요청 | AI 기반 판단 | v2.1.70 |
| \`agent\` | 에이전틱 검증 (도구 사용 가능) | 복잡한 검증 | v2.1.70 |
| \`mcp_tool\` | **MCP 도구 직접 호출** | MCP 서버 도구를 훅에서 바로 사용 | **v2.1.118** |

**\`mcp_tool\` 훅 예시** — 훅에서 Slack MCP 도구를 직접 호출:
...
\`\`\`
비유: 교실 센서가 직접 외부 시스템과 통화!

기존: 훅 → bash 스크립트 → curl 명령으로 Slack API 호출 (코드 작성 필요)
이후: 훅이 곧장 MCP 도구를 호출 → "slack_send_message" 한 줄로 끝
\`\`\`

> 기존에는 MCP 도구를 호출하려면 \`command\` 훅에서 Bash + curl 또는 별도 스크립트를 작성해야 했지만,
> \`mcp_tool\` 훅은 클로드가 평소 사용하는 것과 똑같은 방식으로 MCP 도구를 직접 부를 수 있어요.
