...
window.STUDIO_CONTENT["03-code-search"] = {
  overview: `## 코드 검색 (Glob/Grep) — 코드베이스에서 원하는 것 찾기

10,000줄짜리 프로젝트에서 원하는 코드를 어떻게 찾을까요? 파일을 하나씩 열어보면 하루가 다 갑니다. 클로드 코드에는 **두 가지 강력한 검색 도구**가 있어서 — 도서관 카탈로그처럼 파일을 찾고, 검색 엔진처럼 내용을 뒤질 수 있습니다!

### 이런 상황에서 유용해요
- **파일 위치 파악**: "이 프로젝트에 테스트 파일이 어디 있지?" — Glob으로 패턴 검색
- **코드 내용 추적**: "이 함수를 호출하는 곳이 어디야?" — Grep으로 텍스트 검색
- **복합 탐색**: "컴포넌트 중에서 useState를 쓰는 파일만 찾아줘" — 깔때기 전략(Glob+Grep)

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | Glob, Grep 도구의 역할과 검색 패턴 문법 | 💡 개념 |
| 2 | 파일명 검색, 내용 검색, 깔때기 전략 실습 | 🔧 실습 |
| 3 | 실전 검색 미션과 패턴 치트시트 | 💻 예제 |
...
> **핵심 요약**: Glob은 파일 이름/경로 패턴으로 파일을 찾는 도구입니다. \`*\`(한 단계), \`**\`(모든 하위 폴더), \`{a,b}\`(선택) 등 와일드카드 문법을 조합해서 원하는 파일을 빠르게 검색할 수 있습니다.`
    },
    {
      id: "grep-search",
      title: "Grep (파일 내용 검색)",
...
> **핵심 요약**: Grep은 파일 내용(텍스트)을 검색하는 도구입니다. 정규표현식을 지원하며, \`files_with_matches\`(파일 경로만), \`content\`(매칭 줄 표시), \`count\`(횟수 세기) 3가지 출력 모드를 상황에 맞게 선택합니다.`
    },
    {
      id: "funnel-strategy",
...
> **핵심 요약**: 깔때기 전략은 Glob으로 검색 범위를 먼저 좁히고, Grep으로 내용을 정밀 검색하는 2단계 조합입니다. 전체 프로젝트를 뒤지는 것보다 빠르고 정확한 결과를 얻을 수 있습니다.

### v2.1.162 업데이트 — 전용 검색 도구 명시

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`--tools\`에 Grep/Glob 명시** | 임베디드 검색이 내장된 네이티브 빌드에서 \`--tools\`에 Grep/Glob을 명시하면 **전용 검색 도구를 제공**(이전에는 이름이 조용히 무시됨) | v2.1.162 |`
    }
  ],
