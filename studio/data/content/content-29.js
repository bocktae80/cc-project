...
#### v2.1.140~2.1.153 \`/goal\` 안정성 강화

| 개선 | 설명 | 버전 |
|------|------|------|
| **백그라운드 작업 대기** | \`/goal\` 평가자가 백그라운드 셸이나 위임된 서브에이전트가 **여전히 실행 중이면 발동하지 않음** — 미완료 작업 결과를 놓치고 조건 평가하던 race 해결 | v2.1.143 |
| **훅 차단 시 명확한 메시지** | \`disableAllHooks\` 또는 \`allowManagedHooksOnly\`가 설정된 환경에서 \`/goal\`이 **조용히 hang하던 문제** 수정 — 이제 명확한 에러 메시지 표시 | v2.1.140 |
| **API key 환경에서 자동 비활성** | \`ANTHROPIC_API_KEY\` / \`apiKeyHelper\` / \`ANTHROPIC_AUTH_TOKEN\`이 설정되면 \`/schedule\`, \`/goal\` 관련 알림이 자동 비활성 (커넥터 정책과 통일, 튜토리얼 11 참조) | v2.1.139 |

#### 백그라운드 작업과의 race 회피 (v2.1.143)

\`\`\`
이전 (v2.1.142 이하)
  Turn 5: 백그라운드로 npm test 실행 (5초 소요 예상)
  Turn 5 즉시: /goal 평가자 발동 → "테스트 결과 없음 → 조건 미충족" 잘못 판정
  Turn 6: 같은 명령 재실행... 무한 반복 가능

v2.1.143+
  Turn 5: 백그라운드로 npm test 실행
  Turn 5: /goal 평가자가 background_tasks 존재 확인 → 대기
  Turn 6: 백그라운드 완료 후 평가 → 정확한 조건 검증
\`\`\`

> 자율 루프의 가장 큰 함정 중 하나가 해결됐어요. CI에서 백엔드/프론트 빌드를 동시에 돌리는 시나리오에서 특히 체감 차이가 큽니다.`
    }
  ],
