...
#### v2.1.127~2.1.139 개선 모음

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`/goal\` 신규 커맨드** | 완료 조건을 설정하면 **턴을 넘어가며 자율 진행** — interactive / \`-p\` / Remote Control 모두 지원 (\`/loop\`는 주기 기반, \`/goal\`은 조건 기반) | v2.1.139 |
| **API key 충돌 시 \`/schedule\` 비활성** | \`ANTHROPIC_API_KEY\` / \`apiKeyHelper\` / \`ANTHROPIC_AUTH_TOKEN\`이 설정되면 \`/schedule\`, Remote Control, claude.ai MCP 커넥터, 알림 환경설정이 자동 비활성화 — 사용하려면 API key 해제 후 Claude.ai 로그인만 활성 | v2.1.139 |
| **\`CronList\` 출력 정정** | \`CronList\`가 qualifier(반복/1회성)와 예약된 prompt를 누락하던 문제 수정 | v2.1.136 |

#### \`/goal\` vs \`/loop\` 선택 가이드

\`\`\`
/loop "5분마다 ./scripts/health.sh 결과 보고"
  → 주기적으로 동일 작업 반복
  → 시간 기반: "언제까지인지" 불명확
  → 적합: 외부 시스템 상태 감시

/goal "전체 테스트 통과할 때까지 실패 케이스 고쳐"
  → 완료 조건 만족 시 자동 종료
  → 조건 기반: 모델이 "끝났음" 자가 판단
  → 적합: 결과가 검증 가능한 자율 실행
\`\`\`
...
| 시나리오 | 권장 도구 |
|---------|----------|
| "1시간마다 PR 리뷰 큐 확인" | \`/loop\` (시간 기반) |
| "테스트 통과까지 실패 원인 고치며 반복" | \`/goal\` (조건 기반) |
| "오후 4시 마감 임박 알림" | \`CronCreate\` (1회성 알림) |
| "보안 스캔 high 이슈 0건까지" | \`/goal\` (조건 기반) |

> **상세 패턴**: 29 — /goal 자율 목표(v2.1.139)에서 다룹니다.

#### API key 환경에서 스케줄 기능 비활성 정책 (v2.1.139)
...
→ /loop과 CronCreate는 영향 없음 (로컬 세션 스코프)
\`\`\`

\`\`\`
비유: 회사 계정으로 로그인 시 개인 알림/예약 기능 비활성

기존: API key + Claude.ai 로그인이 둘 다 있으면 → 둘 다 작동
이후: API key가 있으면 → 개인 스케줄/원격 기능 차단
      → 회사 정책과 개인 자동화 경로 혼선 방지
\`\`\`

> 사내 API key를 설정한 채로 \`/schedule\`이 안 보이면 v2.1.139의 정책 변화입니다. \`unset ANTHROPIC_API_KEY\`를 먼저 시도하세요. \`/loop\`/CronCreate는 영향 없습니다.
