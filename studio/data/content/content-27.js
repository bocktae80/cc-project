...
### v2.1.169 업데이트 — /cd로 캐시 유지하며 디렉토리 이동

| 기능 | 설명 | 버전 |
|------|------|------|
| **\`/cd\` 명령** | 세션 중간에 작업 디렉토리를 바꿔도 **프롬프트 캐시가 유지됨**. 예전엔 디렉토리를 옮기려면 새 세션을 열어야 했고 그때 캐시가 초기화됐지만, 이제 \`/cd\`로 **캐시 적중(cache hit)을 보존**하며 이동할 수 있어 비용·지연을 아낌 | v2.1.169 |`
    }
  ],
