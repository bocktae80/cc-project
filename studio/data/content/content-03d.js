...
클로드 코드는 겉으로 보면 대화창이지만, 안에서는 API 호출, 도구 실행, 파일 읽기, 권한 확인 등 엄청나게 많은 일이 벌어집니다. **디버그 기능**은 이 내부 동작을 볼 수 있게 해주는 도구입니다.

### 이런 상황에서 유용해요
- **원인 추적**: "클로드가 갑자기 느려졌는데 왜지?" — \`--debug\` 로그에서 API 호출 횟수와 응답 시간 확인
- **실시간 진단**: "지금 세션 상태가 정상인가?" — \`/debug\`로 컨텍스트 윈도우 사용률 즉시 확인
- **도구 에러 해결**: "파일 읽기가 안 되는데 뭐가 문제야?" — 로그에서 \`[ERROR]\` 검색으로 원인 파악

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | \`--debug\` 플래그와 \`/debug\` 커맨드의 역할과 차이점 | 💡 개념 |
| 2 | 디버그 모드 세션 시작, 로그 분석, 실시간 진단 실습 | 🔧 실습 |
| 3 | 문제 해결 워크플로우와 grep 필터링 치트시트 | 💻 예제 |
...
> **핵심 요약**: \`--debug\` 플래그는 세션 시작 시 켜두면 모든 내부 동작을 파일에 기록하는 블랙박스입니다. 로그는 \`~/.claude/debug/\`에 저장되며, 카테고리 필터(\`"api"\`, \`"!render"\`)로 원하는 부분만 기록할 수도 있습니다.`
    },
    {
      id: "debug-command",
      title: "/debug 커맨드 (계기판)",
...
> **핵심 요약**: \`/debug\`는 세션 중간에 입력하면 현재 상태를 즉시 요약해주는 계기판입니다. 로그 파일 경로, 에러/경고 요약, 최근 로그 20줄을 보여주며, 문제를 빠르게 감지하는 데 유용합니다.`
    },
    {
      id: "log-analysis",
      title: "로그 분석하기",
...
> **핵심 요약**: 로그 분석은 \`grep\`으로 필요한 부분만 필터링하는 것이 핵심입니다. \`[ERROR]\`, \`[WARN]\`, \`[API:request]\` 등 레벨과 카테고리로 검색하면 문제 원인을 빠르게 찾을 수 있습니다.

### v2.1.169 업데이트 — 안전 모드 트러블슈팅

| 기능 | 설명 | 버전 |
|------|------|------|
| **\`--safe-mode\` / \`CLAUDE_CODE_SAFE_MODE\`** | 모든 커스터마이즈(CLAUDE.md·플러그인·스킬·훅·MCP 서버)를 끈 채로 클로드 코드를 시작 — "내 설정 중 뭐가 문제지?"를 가릴 때 깨끗한 기준점을 만들어 줌. 안전 모드에서 멀쩡하면 원인은 커스터마이즈 쪽 | v2.1.169 |`
    }
  ],
