...
| 기능 | 설명 |
|------|------|
| **\`claude agents\` agent view (Research Preview)** | running / blocked / done 모든 Claude Code 세션을 **단일 리스트**에서 보는 통합 뷰. 백그라운드 + 로컬 + 원격 세션 한 곳에서 관리 (v2.1.139) |
| **\`/goal\` 명령어** | 완료 조건을 설정하면 **턴을 넘어가며 자율 진행**, elapsed/turns/tokens 오버레이 표시 (interactive / \`-p\` / Remote Control 지원) (v2.1.139) |
| **\`claude plugin details <name>\`** | 플러그인의 **컴포넌트 인벤토리 + 세션당 예상 토큰 비용** 표시 (v2.1.139) |
| **\`/scroll-speed\` 명령어** | 마우스 휠 스크롤 속도를 라이브 미리보기로 튜닝 (v2.1.139) |
| **트랜스크립트 뷰 네비게이션** | \`?\`(키 도움말), \`{\`/\`}\`(사용자 프롬프트 점프), \`v\`(단축키 패널 토글) (v2.1.139) |
| **\`/mcp\` Reconnect 자동 재로드** | Reconnect가 \`.mcp.json\` 편집을 재시작 없이 반영. 실패 시 HTTP 상태/URL 표시 (v2.1.139) |
| **\`/context all\` 토큰 정확도** | per-skill 토큰 추정이 **모델 토크나이저를 사용**하고 반올림 값으로 표시 (v2.1.139) |
| **\`/context\` 플러그인 출처 표시** | 플러그인이 제공한 스킬에 대해 **제공자 플러그인 이름**을 함께 표시 (v2.1.139) |
| **\`claude plugin install <name>@<marketplace>\` 자동 갱신** | 마켓플레이스를 자동으로 새로고침/재시도 후에 "not found" 보고 (v2.1.139) |
| **stream idle timeout 수정** | 응답 완료 5분 뒤에 발생하던 잘못된 "stream idle timeout" — 스트림 취소 시 watchdog 타이머가 정리되지 않던 문제 해결 (v2.1.139) |
| **\`/usage\` 주간 리셋 날짜 표시** | 주간 리셋이 시각이 아닌 **달력 날짜**로 표시 (v2.1.136) |
| **\`/insights\` 충돌 수정** | 세션 히스토리에 도구 입력 필드가 잘못된 도구 호출이 있을 때 \`/insights\` 크래시 수정 (v2.1.136) |
| **\`@\` 파일 픽커 100+ 엔트리** | 100개 초과 디렉터리에서 \`@\`-mention 파일 픽커가 파일을 못 찾던 문제 수정 (v2.1.136) |
| **\`/release-notes\` stuck 수정** | 변경로그 새로고침 실패 후 옛 버전에서 \`/release-notes\`가 멈춰 있던 문제 수정 (v2.1.136) |
| **세션 ID 기반 효과 환경변수** | Bash 도구 서브프로세스 환경에 \`CLAUDE_CODE_SESSION_ID\`와 \`CLAUDE_EFFORT\` 주입 (훅 입력의 session_id / effort.level과 매칭) (v2.1.132~133) |
| **\`CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN\`** | 풀스크린 alternate-screen 렌더러를 끄고 터미널 native scrollback에 대화 유지 (v2.1.132) |
...
\`\`\`bash
# Headless 모드에서도 사용 가능
claude -p --goal "보안 스캔 high 이슈 0건까지" "scripts/scan.sh를 반복 실행하며 이슈 수정"
\`\`\`

\`\`\`
비유: GPS 내비게이션 vs 음성 지시

기존(/loop): "5분마다 X 해줘" → 시간/주기 기반
이후(/goal): "도착할 때까지 가" → 조건 기반
            모델이 스스로 "더 해야 함 vs 도착함" 판단
\`\`\`

> **\`/goal\` vs \`/loop\`**: \`/loop\`는 정해진 주기로 반복, \`/goal\`은 모델이 완료 조건 충족 시까지 자율 진행. 결과가 명확히 검증 가능한 작업(테스트 통과, 빌드 성공, 스캔 결과 0건)에 적합합니다. 자세한 패턴은 **29 — /goal 자율 목표**(v2.1.139)에서 다룹니다.

#### \`claude agents\` agent view

| 키 | 동작 |
|----|------|
| Enter | 선택한 세션으로 진입 (텔레포트) |
| d | detach (백그라운드로 유지) |
| k | kill (강제 종료) |
| ? | 단축키 도움말 |
...
상태 컬럼:
- **running** — 모델이 작업 중
- **blocked** — 사용자 응답 대기 (AskUserQuestion, 권한 프롬프트)
- **done** — 완료

> 동시에 5개 이상 세션을 굴리면 \`/usage\`나 \`ps\`로 추적이 어려워집니다. \`claude agents\`는 그 단계에서 가장 먼저 켜야 할 화면입니다.
