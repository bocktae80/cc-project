...
window.STUDIO_CONTENT["04-web-search"] = {
  overview: `## 웹 검색/페치 (WebSearch & WebFetch) — 인터넷 정보 활용하기

클로드 코드는 똑똑하지만, 학습 데이터에는 마감일이 있습니다. **2025년 5월** 이후의 정보는 모릅니다. 하지만 **WebSearch**와 **WebFetch**를 사용하면 — 사서에게 책을 찾아달라고 하듯 최신 정보를 직접 인터넷에서 가져올 수 있어요!

### 이런 상황에서 유용해요
- **최신 정보 탐색**: "React 19에 뭐가 바뀌었어?" — WebSearch로 최신 릴리스 정보 검색
- **공식 문서 분석**: "이 라이브러리 설치 방법 알려줘" — WebFetch로 공식 문서 페이지 분석
- **기술 리서치**: "Tailwind v4 변경사항을 조사해줘" — WebSearch로 찾고 → WebFetch로 상세 분석

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | WebSearch, WebFetch, 도메인 필터링의 역할과 차이점 | 💡 개념 |
| 2 | 최신 정보 검색, 페이지 분석, 리서치 조합 실습 | 🔧 실습 |
| 3 | 실전 리서치 워크플로우 패턴 | 💻 예제 |

> 검색할 때 **현재 연도(2026)**를 검색어에 포함하면 최신 결과를 얻기 좋습니다.
...
> **핵심 요약**: WebSearch는 검색 엔진처럼 검색어로 결과 목록을 가져오는 도구입니다. \`allowed_domains\`/\`blocked_domains\`로 검색 범위를 조절하고, 검색어에 연도와 구체적 키워드를 포함하면 더 좋은 결과를 얻습니다.`
    },
    {
      id: "web-fetch",
...
> **중요**: WebFetch는 **공개 페이지만** 접근할 수 있어요. 로그인이 필요한 사이트는 접근이 실패합니다.
> 인증이 필요한 서비스(Google Docs, Confluence, Jira 등)는 **MCP 서버**를 통해 연결하세요.
> GitHub URL의 경우 \`gh\` CLI를 통해 접근하는 것이 더 안정적입니다.

> **v2.1.83**: WebFetch는 이제 User-Agent를 \`Claude-User\`로 식별합니다. 사이트 운영자가 \`robots.txt\`에서 Claude Code 트래픽을 인식하고 허용/차단할 수 있어요.

> **핵심 요약**: WebFetch는 특정 URL의 웹 페이지를 읽고 분석하는 도구입니다. \`url\`과 \`prompt\`를 함께 제공하며, prompt가 구체적일수록 정확한 결과를 얻습니다. 공개 페이지만 접근 가능하고, 로그인 필요 사이트는 MCP를 사용하세요.`
    },
    {
      id: "domain-filtering",
      title: "도메인 필터링",
...
> **핵심 요약**: 도메인 필터링은 WebSearch 검색 결과를 특정 사이트로 제한(\`allowed_domains\`)하거나 제외(\`blocked_domains\`)하는 기능입니다. 공식 문서만 보거나 품질 낮은 결과를 제외할 때 유용합니다.`
    }
  ],
