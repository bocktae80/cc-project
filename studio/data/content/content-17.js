...
### v2.1.157~163 업데이트 — 플러그인 자동 로드

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`.claude/skills\` 플러그인 자동 로드** | \`.claude/skills\` 디렉토리의 플러그인이 **마켓플레이스 없이 자동 로드** | v2.1.157 |
| **\`claude plugin init <name>\`** | \`.claude/skills\`에 새 플러그인을 **스캐폴딩** | v2.1.157 |
| **\`/plugin\` 인자 자동완성** | 서브커맨드, 설치된 플러그인 이름, 알려진 마켓플레이스의 플러그인을 자동완성 | v2.1.157 |
| **\`/plugin list\`** | 설치된 플러그인 목록 조회, \`--enabled\`/\`--disabled\` 필터 지원 | v2.1.163 |

### v2.1.172 업데이트 — 플러그인 마켓플레이스

| 개선 | 설명 | 버전 |
|------|------|------|
| **\`/plugin\` 마켓플레이스 검색 바** | \`/plugin\` 마켓플레이스 브라우저에 in-pane 검색 바(type-to-filter) 추가 — 스크롤 없이 플러그인 필터 | v2.1.172 |`,
      checklist: [
        "플러그인 검색 방법을 알고 있다",
        "플러그인 선택 기준을 이해한다",
        "인기 플러그인 카테고리를 파악했다",
