...
게임에서 **포탈**을 열면 다른 장소로 순간이동하잖아요? 클로드 코드에서도 똑같이 할 수 있어요!
터미널에서 작업하다가 포탈을 열면 웹브라우저에서 이어하고, 또 다른 포탈을 열면 다시 터미널로 돌아올 수 있어요.

### 이런 상황에서 유용해요
- **긴 작업 맡기기**: "리팩토링 30분 걸리는데 터미널 묶여있기 싫어" — \`&\` prefix로 클라우드에 보내고 터미널 자유!
- **웹 설계 → 구현 전환**: "claude.ai에서 설계 끝났는데 이제 코딩하고 싶어" — \`/teleport\`로 터미널에서 바로 시작
- **이동 중 모니터링**: "회의 가면서 작업 상황 확인하고 싶어" — \`/rc\`로 다른 기기에서 원격 접속

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | & prefix, /teleport, /rc의 역할과 차이점 | 💡 개념 |
| 2 | 클라우드 전송, 세션 이동, 원격 접속 실습 | 🔧 실습 |
| 3 | 텔레포트 워크플로우 + 리모트 컨트롤 활용 | 💻 예제 |
...
>
> **v2.1.118**: \`/color\`가 Remote Control 연결 시 claude.ai/code의 **세션 액센트 색상에 자동 동기화**됩니다. 데스크톱과 모바일/웹의 색상이 한 화면 안에서 일치돼 어떤 세션인지 시각적으로 즉시 구분 가능.
>
> **v2.1.118**: \`--continue\`/\`--resume\`이 \`/add-dir\`로 현재 디렉터리를 추가했던 세션도 찾아냅니다 — 텔레포트로 이동했던 다른 디렉터리에서도 같은 세션을 이어서 열 수 있어요.`,
...
> **핵심 요약**: 텔레포트는 세션을 다른 환경으로 이동시키는 기능입니다. \`&\` prefix로 클라우드에 보내고, \`/teleport\`로 웹에서 터미널로 가져오고, \`/rc\`로 원격 접속합니다. 포탈을 열어 자유롭게 이동하는 게임처럼요!`
    },
    {
      id: "teleport-vs-rc",
...
> **핵심 요약**: \`/teleport\`는 세션 자체를 이동시키는 "이사"이고, \`/rc\`는 세션은 그대로 두고 다른 기기에서 접속하는 "TV 리모컨"입니다. 세션을 완전히 옮기려면 \`/teleport\`, 다른 기기에서 확인/조종만 하려면 \`/rc\`를 쓰세요.`
    }
  ],
