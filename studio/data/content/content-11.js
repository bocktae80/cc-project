...
07-mcp-server에서 MCP 서버를 직접 설정해봤죠? 이번엔 **이미 누군가 설정해둔 도구를 바로 쓰는 법**을 배워요! 학교 공유 프린터처럼, 설치 없이 "인쇄" 버튼만 누르면 되는 세상입니다.

### 이런 상황에서 유용해요
- **Slack 연동**: "Slack 채널 메시지를 읽어줘" — 커넥터 활성화만 하면 바로 사용
- **Notion 연동**: "Notion에서 회의록 찾아줘" — OAuth로 한 번 인증, 그 다음은 자동
- **서비스 조합**: "Slack 메시지를 요약해서 Notion에 정리해줘" — 여러 커넥터를 엮어 한 번에 처리

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | 수동 MCP vs 커넥터 차이, OAuth 인증, ToolSearch | 💡 개념 |
| 2 | 커넥터 활성화 → Slack/Notion 사용 → ToolSearch 실습 | 🔧 실습 |
| 3 | 여러 커넥터를 조합한 실전 워크플로우 | 💻 예제 |
...
| 상황 | 추천 |
|------|------|
| Slack/Notion/Gmail 같은 인기 서비스 | 커넥터 (간편!) |
| 회사 내부 시스템 연동 | 수동 MCP (커스텀 필요) |
| 특별한 설정이 필요한 경우 | 수동 MCP |
| 빠르게 시작하고 싶을 때 | 커넥터 |

> **v2.1.70 기준 제공 커넥터**: Slack, Notion, Gmail, Google Calendar, Figma, Linear, Vercel, AWS Marketplace 등 — 계속 추가 중!

> **핵심 요약**: 수동 MCP는 집 프린터(직접 설치/관리), 커넥터는 학교 공유 프린터(바로 사용). 인기 서비스(Slack, Notion 등)는 커넥터가 간편하고, 회사 내부 시스템은 수동 MCP가 필요합니다.`
    },
    {
      id: "oauth-auth",
...
> **핵심 요약**: OAuth는 "허용" 한 번 클릭으로 인증이 끝나고 자동 갱신됩니다. API 키처럼 직접 발급/저장/만료 관리할 필요가 없어서 간편하고 안전합니다.`
    },
    {
      id: "toolsearch",
...
- **스키마 유실 수정**: 대화가 길어져 컨텍스트가 압축(compaction)되어도, ToolSearch로 불러온 도구의 파라미터 스키마가 유지됩니다. 이전에는 압축 후 배열/숫자 파라미터가 타입 에러를 일으킬 수 있었습니다.

### v2.1.83~2.1.84 개선사항

| 개선 | 설명 |
|------|------|
| **커넥터 \`--print\` 모드** | claude.ai 커넥터(Slack, Gmail 등)가 단일 턴 \`--print\` 모드에서도 사용 가능! 스크립트에서 커넥터 도구 활용 가능 (v2.1.83) |
| **도구 설명 2KB 캡** | MCP 도구 설명이 **2KB로 제한**되어 컨텍스트 절약 (v2.1.84) |
| **로컬/커넥터 중복 제거** | 같은 서버가 로컬+커넥터에 모두 있으면 **로컬 우선**, 중복 연결 방지 (v2.1.84) |
...
> 예전에는 Slack 열기 → 복사 → Notion 열기 → 붙여넣기 → 정리... 이 과정이 필요했지만, 커넥터로 한 번에 처리할 수 있습니다!`,
      checklist: [
        ...
        "v2.1.70 기준 제공 커넥터 종류(Slack, Notion, Gmail, Calendar 등)를 안다"
      ]
    }
  ],
