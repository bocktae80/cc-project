...
window.STUDIO_CONTENT["02-file-operations"] = {
  overview: `## 파일 읽기/쓰기 — 파일을 자유자재로 다루기

클로드 코드는 파일을 직접 읽고, 만들고, 고칠 수 있습니다. 도서관 사서가 책을 찾아주고, 새 책을 꽂고, 오탈자를 고치듯 — 3가지 도구로 모든 파일 작업을 처리해요!

### 이런 상황에서 유용해요
- **코드 확인**: "이 설정 파일에 뭐가 들어있지?" — Read로 즉시 확인
- **새 파일 생성**: "이 구조의 컴포넌트를 만들어줘" — Write로 새 파일 생성
- **버그 수정**: "여기서 포트 번호만 바꿔줘" — Edit로 정확한 부분만 수정

### 이 튜토리얼에서 배우는 것
| 순서 | 내용 | 탭 |
|------|------|-----|
| 1 | Read, Write, Edit 도구의 역할과 차이점 | 💡 개념 |
| 2 | 파일 읽기, 만들기, 수정하기 실습 | 🔧 실습 |
| 3 | 상황별 올바른 도구 선택 가이드 | 💻 예제 |
...
- **병렬 파일 읽기**: 여러 파일을 동시에 읽을 때, 하나가 실패해도 나머지는 정상 처리됩니다
- **대용량 결과 처리**: 50,000줄 이상의 큰 결과는 자동으로 디스크에 저장되어 메모리를 절약합니다

### v2.1.89~2.1.92 도구 개선

| 개선 | 설명 | 버전 |
|------|------|------|
| **Edit, Read 없이 작동** | Bash에서 \`sed -n\`이나 \`cat\`으로 본 파일에 별도 Read 없이 바로 Edit 사용 가능 | v2.1.89 |
| **Edit 앵커 짧아짐** | Edit 도구가 더 짧은 \`old_string\` 앵커를 사용 → 출력 토큰 절감 | v2.1.91 |
| **Write diff 60% 빠름** | 탭/\`&\`/\`$\` 포함 파일에서 Write 도구 diff 연산 60% 속도 향상 | v2.1.92 |

### v2.1.86 토큰 절감 개선

- **compact 줄번호 포맷**: Read 출력이 더 간결한 줄번호 형식을 사용하여 토큰 사용량이 줄어듭니다
- **중복 re-read 제거**: 같은 파일을 이미 읽었고 변경되지 않았다면, 중복 읽기를 자동 스킵하여 토큰을 절약합니다

\`\`\`
비유: 도서관에서 같은 책을 또 빌릴 때

기존: "이 책 보여주세요" → 매번 처음부터 다시 보여줌 (토큰 낭비)
이후: "이 책 아까 봤는데?" → "바뀐 게 없으니 패스!" (자동 스킵)
\`\`\`

> **핵심 요약**: Read는 파일을 읽기만 하는 100% 안전한 도구입니다. offset/limit으로 큰 파일도 원하는 부분만 읽을 수 있고, 텍스트·이미지·PDF·노트북까지 다양한 형식을 지원해요. v2.1.86부터 compact 포맷과 중복 읽기 제거로 토큰 효율이 향상되었습니다.`
    },
    {
      id: "write-tool",
...
> **핵심 요약**: Write는 파일을 새로 만들거나 통째로 교체하는 도구입니다. 기존 파일이 있으면 내용이 전부 사라지므로, 일부 수정에는 Edit를 사용하세요.`
    },
    {
      id: "edit-tool",
      title: "Edit (부분 수정)",
