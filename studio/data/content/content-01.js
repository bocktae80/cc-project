...
- **세션 간 자동 저장**: 자동 메모리가 세션이 끝날 때 자동으로 저장되어, 다음 세션에서 이전 맥락을 바로 이어갈 수 있습니다
- **워크트리 간 공유**: git worktree를 사용하더라도 메모리가 프로젝트 단위로 공유됩니다 (자세한 내용: 09-worktree 튜토리얼)
...
메모리 시스템과 함께 알아두면 좋은 기능들입니다:

| 튜토리얼 | 관련 내용 |
|----------|----------|
| **08-skills-commands** | \`/memory\` 커맨드로 메모리 파일 편집 |
| **09-worktree** | 워크트리 간 메모리 공유 동작 방식 |
| **10-cli-master** | \`claude config\` 등 CLI 설정 관리 |`,
      checklist: []
    }
  ],
