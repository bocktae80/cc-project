...
## 현재 단계
- **Phase:** 전체 완료 (M0~M6 + 인프라 + M7~M18 유지보수)
- **진행률:** 31/31 튜토리얼 완료
- **CLI 기준 버전:** v2.1.178 (31개 전부 최신, 갱신 필요 0개)
- **최근 작업:** M18 완료 — v2.1.178 갱신(분배형, 신규 튜토리얼 없음). M17 이후 누적된 v2.1.170~178 9개 패치를 **ultracode 동적 워크플로우**로 분석 — 6개 버전 병렬 추출 → 후보 46건 실제 콘텐츠 대조 적대 검증(UPDATE 32/SKIP 14/신규 0/Critic 누락 0). 12개 콘텐츠 분배(19/10/05/08/30/16/20/06/17/01/13/28): ⭐⭐⭐ **`Tool(param:value)` 권한 룰 문법**(`Agent(model:opus)` 예시, 19)·**Claude Fable 5**(Mythos-class) 신규 모델(10)·**중첩 서브에이전트 5단계**(05); ⭐⭐ availableModels/enforceAvailableModels 거버넌스 강화·중첩 `.claude/` 해석·auto mode 서브에이전트 사전평가·`language`/`footerLinksRegexes`/`/cd` 브랜치·hook `if` 경로 매칭·VSCode usage/PowerShell·`/rc` 진단 등. 백로그 BL-01(13)·BL-02(30/01/16) 재개 조건 충족 동반 반영. **Proofline PASS**(PFS 94/MQS 92.0/Review 90/RedTeam DTS 89.55, major 0). version-track +36(794→830 유실 0), 31개 basedOn 2.1.178 동기화, 31개 콘텐츠 구문 검증 통과 + 갭 0 (2026-06-16)
- **다음 사이클(M19):** 데일리 자동 체크가 새 버전 감지 시 `workbook/log/update-checks/`에 리포트 생성 → 검토 후 분배. 잔존 백로그 BL-01(07/11)·BL-02(18) 재개 조건 충족 시 함께 반영
...
| M5: 확장 기능 | 2/2 | 완료 |
| M6: 심화 활용 | 7/7 | 완료 |
| M7~M18: 유지보수 | v2.1.92→2.1.178 | 완료 (신규 5개: 26/27/28/29/30) |
